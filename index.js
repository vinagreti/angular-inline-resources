...
function inlineStyle(content, urlResolver) {
  return content.replace(/styleUrls:\s*(\[[\s\S]*?\])/gm, function(m, styleUrls) {
    ...
    return 'styles: ['
      + urls.map(styleUrl => {
          const fileLen = styleUrl.length;
          const fileExtension = styleUrl.substring(fileLen - 5, fileLen)
          ...
          if(fileExtension == '.scss'){
            shortenedStyle = getCssFromSass(shortenedStyle)
              .replace(/([\n\r]\s*)+/gm, ' ')
              .replace(/"/g, '\\"');
          }
...
/**
 * Compile sass on the fly by string not by fie
 * @param sass {string} The sass file's content.
**/
function getCssFromSass(sassStr) {
